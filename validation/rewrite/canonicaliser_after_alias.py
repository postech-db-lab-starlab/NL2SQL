...
def read_schema(schema_filename, add_db2_columns=False):
    schema = {}
    all_words = set()
    with open(schema_filename) as schema_file:
        for line in schema_file:
            ...
            if len(fields) > 1:
                table = fields[0].upper()
                column = fields[1].upper()
                schema.setdefault(table, set()).add(column)
                all_words.add(table)
                all_words.add(column)
    if add_db2_columns:
        for i in range(5):
            column = 'DB2_TMP_COL{}'.format(i)
            for table in schema.keys():
                schema[table].add(column)
            all_words.add(column)
    return schema, all_words

def standarise_file(filename, schema_filename, log, overwrite, skip, nonjson, write_path=None, db2_rewritten=False):
    schema = read_schema(schema_filename, add_db2_columns=db2_rewritten)
    ...
    with open(new_path, 'w') as write_file:
        if nonjson:
            ...
            to_print = json.dumps(final_data, indent=4, sort_keys=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--db2_rewritten', help='Input queries are rewritten by IBM DB2 query rewriter', action='store_true')
    ...
    if not (args.testadv or args.testgeo or args.testatis or args.testscholar or args.testyelp):
        for line in sys.stdin:
            ...
            standarise_file(line.strip(), args.fields, args.log, args.overwrite, skip, args.nonjson, args.write, db2_rewritten=args.db2_rewritten)
    else:
        sample_queries = [
...
###
        ...
        schema = read_schema(args.fields, add_db2_columns=args.db2_rewritten)
