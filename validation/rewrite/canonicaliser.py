...
def standardise_aliases(query, schema):
    ...
    for i, word in enumerate(tokens):
        ...
        if prev_pos != current_subquery[0]:
            aliases_in_name[current_subquery[0]] = aliases_in_name[prev_pos].copy()
        if word == "FROM":
        ...
        if word in schema[0] and not seen_where[current_subquery[0]]:
            ...
            if has_alias:
                ...
                aliases_in_name[current_subquery[0]][tokens[i+2]] = alias
                tokens[i+2] = alias
            else:
                aliases[current_subquery[0], word] = alias
                aliases_in_name[current_subquery[0]][word] = alias
...
###                    print("New alias", current_subquery[0], tokens[i], alias)
                    ...
                    aliases_in_name[current_subquery[0]][tokens[i]] = alias
                tokens[i] = alias
    ...
    for i, word in enumerate(tokens):
        ...
        if len(parts) == 2:
            ...
            #print(parts[1], field_aliases)
            #'''
            if parts[1] in field_aliases:
                if tokens[i+1].upper() == 'AS' and not tokens[i].upper().startswith('_C'):
                    pass
                else:
                    tokens[i] = "." + field_aliases[parts[1]]
            '''
            if parts[0] in aliases:
                #table = aliases[parts[0]]
                #table = aliases_in_name[current_subquery[0]][parts[0]]
                field = parts[1]
